...
from sim_db import ALLOWED_STATUS, DEFAULT_DB_PATH, add_sim_item, del_case_by_job_id, init_sim_db, list_items, list_view, mark_done, resolve_job_id, upd_case_by_job_id
...
class SecurityPolicy:
    def __init__(self, token: str, default_db_path: str, allowed_db_path: str | None = None, allowed_base_dir: str | None = None) -> None:
        self.token = token
    ...
    def is_authorized(self, auth_header: str | None) -> bool:
        return bool(auth_header and auth_header.startswith("Bearer ") and auth_header[len("Bearer ") :] == self.token)
...
class SimDbRequestHandler(BaseHTTPRequestHandler):
            ...
            try:
                ...
                if urlsplit(self.path).path == "/cases/summary":
                    with self.server.mutation_lock:
                        rows = list_view(db_path=db_path, status=query.get("status"), run_host=query.get("run_host"), sort_by=query.get("sort_by", "updated_at"), desc=query.get("order", "desc").lower() != "asc", limit=int(query["limit"]) if query.get("limit") else None)
                ...
                with self.server.mutation_lock:
                    ...
                    if case_ref:
                        job_id = self._resolve_job_id_from_table(data, case_ref)
                        item = data[job_id]
                        self._json(HTTPStatus.OK, {"db_path": db_path, "case": item["case"], "item": item})
                        return
    ...
    def do_DELETE(self) -> None:  # noqa: N802
        ...
        try:
            ...
            with self.server.mutation_lock:
                ...
                job_id = self._resolve_job_id_from_table(table, case_ref)
                del_case_by_job_id(db_path, job_id)
            self._json(HTTPStatus.OK, {"ok": True, "db_path": db_path, "job_id": job_id})
        except Exception as exc:
            self._json(HTTPStatus.BAD_REQUEST, {"error": str(exc)})
    ...
    def _create_case(self, payload: dict[str, Any]) -> None:
        try:
            ...
            db_path = self.server.policy.resolve_db_path(payload.get("db_path"))
            with self.server.mutation_lock:
                add_sim_item(case=payload["case"], inp=payload.get("inp"), input_files=payload.get("input_files"), bin_name=payload["bin_name"], status=payload["status"], db_path=db_path, note=payload.get("note"), work_dir=payload.get("work_dir"), extra_params=payload.get("extra_params"))
                from sim_db import derive_job_id
                primary_inp = payload.get("inp")
                files = payload.get("input_files") or ([] if primary_inp is None else [primary_inp])
                resolved_work_dir = payload.get("work_dir") or os.getcwd()
                job_id = derive_job_id(case=payload["case"], work_dir=resolved_work_dir, inp=primary_inp, input_files=files)
                run_host = payload.get("run_host")
                if run_host:
                    upd_case_by_job_id(db_path, job_id, {"run_host": str(run_host)})
            self._json(HTTPStatus.OK, {"ok": True, "db_path": db_path, "case": payload["case"], "job_id": job_id})
    ...
    @staticmethod
    def _resolve_job_id_from_table(data: dict[str, dict[str, Any]], case_ref: str) -> str:
        if case_ref in data:
            return case_ref
        return resolve_job_id(list(data.values()), case=case_ref)
    ...
    def _update_case(self, case_ref: str, payload: dict[str, Any]) -> None:
        try:
            ...
            with self.server.mutation_lock:
                ...
                if fields:
                    upd_case_by_job_id(db_path, job_id, fields)
                item = list_items(db_path)[job_id]
            self._json(HTTPStatus.OK, {"ok": True, "db_path": db_path, "job_id": job_id, "case": item["case"], "updated": sorted(fields.keys())})
        except Exception as exc:
            self._json(HTTPStatus.BAD_REQUEST, {"error": str(exc)})
    ...
    def _query_dict(self) -> dict[str, str]:
        ...
        return {k: vals[0] for k, vals in query.items() if vals}
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--allowed-db-path", default=None)
    p.add_argument("--allowed-base-dir", default=None)
...
def main(argv: list[str] | None = None) -> int:
    ...
    policy = SecurityPolicy(token=token, default_db_path=args.db, allowed_db_path=args.allowed_db_path, allowed_base_dir=args.allowed_base_dir)
